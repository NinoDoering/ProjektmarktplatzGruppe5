...
public class TeamMapper {
		...
		public Team inser (Team t){
			...
			try {
				...
				if (rs.next()) { 
			       ...
			       stmt.executeUpdate(" INSERT INTO Team (id, teamName, mitgliederAnzahl )" 
			    		   + "VALUES ( " + t.getId()+ " ,'" +t.getTeamName()+ "','" + t.getMitgliederAnzahl()+ "')");
			}
		}
		...
		public Vector <Team> findAll() {
			...
			try {
				...
				ResultSet rs = stmt.executeQuery("SELECT id, teamName, mitgliederAnzahl"
		...
		try {
			...
			ResultSet rs = stmt.executeQuery("SELECT id, teamName, mitgliederAnzahl" 
			 +" FROM team " + "WHERE bezeichnung LIKE '" + teamName + "' ORDER BY teamName ");
		...
		public Team update(Team t){
			...
			try {
				...
				stmt.executeUpdate("UPDATE team " + "SET teamName=\"" + t.getTeamName()
