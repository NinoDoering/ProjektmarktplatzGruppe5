...
public class BewerbungenAufAusschreibungSeite extends VerticalPanel{
		...
		Button zurueckButton = new Button("Zur�ck");
		...
		public BewerbungenAufAusschreibungSeite(int idAusschreibung, final Navigator navigator, final RoleManagement roleManagement){
			...
			/**
			 * TextColumns anlegen und der Celltable hinzuf�gen		
			 */
	 ...
	 * Speichert die ausgew�hlte Zeile des Celltables
	 */
	...
	/**
	 * Hinzuf�gen der Buttons zum ButtonPanel
	 */
	 ...
	 * �ffnen der DialogBox BewerbungBewerten
	 */
	bewerbungBewertenButton.addClickHandler(new ClickHandler() {
		public void onClick(ClickEvent event) {
			if(selectionModel.getSelectedObject() == null){
				Window.alert("Bitte w�hle zuerst eine Bewerbung aus.");
			} else{
	...
	/**
	 * Click Handler um zur Hauptseite zur�ck zu gelangen		
	 */
...
//			RootPanel.get("Anzeige").add(new EigeneAusschreibungen(roleManagement, navigator));	
	...
	partnerprofilAnzeigen.addClickHandler(new ClickHandler() {
		...
		/**
		 * Impelemtierung der Inneren privaten Klasse um Celltable mit einheitlichem Datensatz zu bef�llen
		 *
		 */
		 ...
		 * Bei erfolgreichem Callback wird ein Vector mit Bewerbungen als result zur�ckgibt.
		 * 
		 * @author Tom 
		 *
		 */
		private class GetBewerbungenCallback implements AsyncCallback<Vector<Bewerbung>>{
			...
			public void onSuccess(Vector<Bewerbung> result) {
				...
				for(int i=0;i<bewerbungen.size();i++){
					...
					/**
					 *  Die Bewertung wird beim Callback zu der �bergebenen Bewerbung als
					 * result zur�ckgegeben
					 */
					...
					greetingService.getBewertungByBewerbung(bewerbungen.get(i), new AsyncCallback<Bewertung>(){
						...
						public void onSuccess(Bewertung result) {
							...
							/**
							 * Die Organisationseinheit wird zu der �bergebenen Beteiligung als
							 * result zur�ckgegeben
							 *
							 */
					...
					/**
					 * Die Organisationseinheit wird zu der �bergebenen Bewerbung als
					 * result zur�ckgegeben
					 */
					greetingService.getOrganisationseinheitById(bewerbungen.get(i).getIdOrganisationseinheit(), new AsyncCallback<Organisationseinheit>() {
						...
						public void onSuccess(Organisationseinheit result) {
							/**
							 * Pr�fung ob die Organisationseinheit eine Person, ein Team oder ein Unternehmen ist.
					...
					/**
					 * Das localMix-Objekt wird dem Vector hinzugef�gt
					 */
					mix.add(localMix);
